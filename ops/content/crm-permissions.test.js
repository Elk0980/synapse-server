...
test('CRM proxy restricts pipelines to the owner and preserves company-scoped editing', async (t) => {
  ...
  await t.test('company mail and deal representatives never inherit another company customer base',async()=>{
    const settings=await crm(owner,'GET','/company-email?companyCode=alvi');
    assert.equal(settings.status,200,JSON.stringify(settings.body));
    assert.equal(settings.body.recipient,qaEmailSettings.alviRecipient);
    assert.deepEqual(settings.body.status.companies.map(c=>c.code),['alvi']);
    assert.equal(Object.hasOwn(settings.body,'password'),false);
    assert.equal((await crm(editor,'GET','/company-email?companyCode=alvi')).status,403);
    assert.equal((await crm(owner,'GET','/company-email')).status,400);
    assert.equal((await crm(owner,'PUT','/company-email?companyCode=alvi',{recipient:'new-qa@example.test'})).status,200);
    assert.equal((await crm(owner,'GET','/company-email?companyCode=avokado')).body.recipient,qaEmailSettings.avokadoRecipient);
    assertNoQueuedOrAttemptedEmail();
    const before=await crm(owner,'GET','/deals/criteria?companyCode=avokado');
    assert.notEqual(before.body.stages[0].label,'Новый запрос');
    assert.ok(!before.body.stages.some(s=>s.label==='Проверка результата'));
    const customer=await crm(owner,'POST','/companies?companyCode=alvi',{code:'qa-studio-customer',name:'QA Studio'});
    const privatePerson=await crm(owner,'POST','/contacts?companyCode=avokado',{name:'Customer of another studio'});
    const person=await crm(owner,'POST','/contacts?companyCode=alvi',{name:'Owner representative',companyLink:{companyId:customer.body.id,role:'Собственник'}});
    assert.equal(person.status,201,JSON.stringify(person.body));
    const created=await crm(owner,'POST','/deals?companyCode=alvi',{companyId:customer.body.id,title:'Services agreement'});
    assert.equal(created.status,201);const id=created.body.id;
    assert.deepEqual(created.body.contacts,[], 'a company relation alone is not deal participation');
    assert.equal((await crm(owner,'PATCH',`/deals/${id}/contacts/${person.body.id}?companyCode=alvi`,{name:'Must not change'})).status,404);
    const participantPath=`/deals/${id}/participants?companyCode=alvi`;
    assert.equal((await crm(owner,'POST',participantPath,{contactId:privatePerson.body.id})).status,404);
    assert.equal((await crm(owner,'POST',participantPath,{contactId:person.body.id,role:'Собственник'})).status,200);
    assert.equal((await crm(owner,'GET',`/deals/${id}?companyCode=alvi`)).body.contacts[0].name,'Owner representative');
    const added=await crm(owner,'POST',participantPath,{newContact:{name:'New representative',phone:'+79990008888'},role:'Согласующий'});
    assert.equal(added.status,200,JSON.stringify(added.body));assert.equal(added.body.total,2);
    assert.equal((await crm(owner,'DELETE',`/deals/${id}/participants/${person.body.id}?companyCode=alvi`)).status,200);
    assert.equal((await crm(owner,'GET',`/contacts/${person.body.id}?companyCode=alvi`)).status,200,'removing participation preserves the person');
    const prefilled=await crm(owner,'POST','/deals?companyCode=alvi',{companyId:customer.body.id,title:'Prepared from contact',contactId:person.body.id});
    assert.equal(prefilled.status,201);assert.equal(prefilled.body.totalContacts,1);
    assert.equal((await crm(owner,'POST',`/deals/${id}/participants?companyCode=avokado`,{contactId:privatePerson.body.id})).status,404);
  });

  await t.test('commercial catalog and finance are owner-only; proposals preserve billing and module dependencies',async()=>{
    for(const session of [editor,observer,viewer])for(const path of ['/catalog','/finances?companyCode=synapse-business'])assert.equal((await crm(session,'GET',path)).status,403);
    assert.equal((await crm(owner,'POST','/companies',{code:'synapse-business',name:'Synapse Бизнес'})).status,201);
    assert.equal((await crm(owner,'GET','/catalog?companyCode=alvi')).status,400);
    ...
    const company=await crm(owner,'POST','/companies?companyCode=synapse-business',{code:'qa-commercial',name:'QA Commercial'});assert.equal(company.status,201);
    let d=(await crm(owner,'POST','/deals?companyCode=synapse-business',{companyId:company.body.id,title:'QA Catalog'})).body;
    const path=`/deals/${d.id}/offer?companyCode=synapse-business`;
    ...
    const f=await crm(owner,'POST','/finances?companyCode=synapse-business',{requestId:'qa-finance',type:'income',state:'planned',date:'2026-09-16',amount:50000,category:'Партнёры',dealId:d.id});assert.equal(f.status,200,JSON.stringify(f.body));
    const finance=await crm(owner,'GET','/finances?companyCode=synapse-business&from=2026-09-01&to=2026-09-30');assert.equal(finance.body.totals.income,0);assert.equal(finance.body.totals.plannedIncome,50000);
