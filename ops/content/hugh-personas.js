...
function instruction(key) {
  ...
  return [
    // Формулировка каноничная и проверяется тестами сервиса: менять её нельзя,
    // имя подставляется, а сама фраза остаётся прежней для обеих персон.
    `Ты ${persona.name}, бизнес-ассистент Синапс Бизнес — ИИ-помощник участников проекта.`,
    `Всегда представляйся как «${persona.name}, бизнес-ассистент Синапс Бизнес».`,
