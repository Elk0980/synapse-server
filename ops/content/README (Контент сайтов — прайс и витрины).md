...
## Аккаунты, права и реестр сайтов

Аккаунты хранятся в `content.sqlite`; `AUTH_USERS` используется только до появления маркера
`auth_users_imported_v1`. Сессия содержит лишь `uid`, версию сессии, срок и CSRF-токен, а актуальные роль,
компании и права загружаются из SQLite на каждом запросе. Legacy-ключ редактора жёстко ограничен документами
ALVI и Авокадо и не открывает API аккаунтов или managed-сайтов.

Статусы bootstrap-реестра подтверждены `caddy/Caddyfile`: Synapse Бизнес, ТайСабай, ALVI и Палитра лав
подключают snippet `draft` с `X-Robots-Tag: noindex, nofollow`, поэтому имеют статус `draft`. Основной контур
Авокадо snippet `draft` не подключает и имеет статус `published`. НовыйЭтап не импортируется.

`PRICE_HIERARCHY=APPROVED_NEXT_TASK`: следующим отдельным изменением предлагается дерево
«категория → подкатегория → позиция», ручная сортировка и принадлежность позиции нескольким категориям.
Вместе с ним на рассмотрение Owner'а предлагаются универсальная карточка, режимы цены, типы и характеристики,
варианты, доплаты, статусы, поиск и фильтры. Остатки/SKU, массовый CSV/YML и комплекты следует оставить на потом.
`PRICE_PROPOSAL_STATUS=PROPOSAL_NOT_IMPLEMENTED`: формат документов, валидаторы и публичные renderer'ы этой
задачей не меняются.

`CHAT_PERMISSION_ENFORCEMENT=PARTIAL`: кабинет учитывает `chat.view`, но chat-admin остаётся защищён собственным
ключом. `NEXT_REQUIRED=отдельная session-aware интеграция ops/chat`.
