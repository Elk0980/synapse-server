...
Later owner-requested framing revision: the gallery now displays the existing public source photos through 1024 × 1024 SVG frames. In-image text and outer borders are excluded by documented viewport crops, with uniform scaling inside each pair and no retouching. Original full collages remain in the repository; no derivative image is uploaded in this revision. The enlarged dialog uses the same framing. This replaces the initial full-frame/no-crop display decision above. See AVOKADO3-CONTACTS-AND-CROPS-20260915.md and results-crop-recipe.json for the current rule and evidence.
