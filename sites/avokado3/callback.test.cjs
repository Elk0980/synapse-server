...
test('callback validates consent and fields before creating a request', () => {
  for (const [field, value] of [['name', '   '], ['name', 'я'.repeat(81)], ['contact', 'hello'],
    ['contact', '123'], ['contact', '1'.repeat(16)], ['comment', 'я'.repeat(1001)], ['consent', false]]) {
...
test('only a successful JSON CRM response with a positive id confirms a callback', async () => {
  for (const status of [200, 201]) {
    let request;
    const id = await send(transport(async (...args) => {
      request = args;
      return {status, ok: true, json: async () => ({id: 12, deduplicated: status === 200})};
  ...
  for (const result of [{}, {id: 0}, {id: -1}, {id: '12'}, {emailDelivered: true}]) {
    await assert.rejects(send(transport(async () => ({status: 200, ok: true, json: async () => result})), {}));
  }
  ...
  await assert.rejects(send(transport(async () => ({status: 200, ok: true, json: async () => {throw Error('HTML response');}})), {}));
});
...
function formFixture(fetch) {
  ...
  const button = {disabled: false, textContent: 'Оставить заявку'};
...
test('pending prevents duplicate requests; accepted callback resets the form without email claims or analytics PII', async () => {
  ...
  assert.match(fixture.status.textContent, /^Заявка принята/);
