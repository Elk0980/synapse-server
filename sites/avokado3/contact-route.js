...
(function (host, factory) {
  ...
  if (host && host.document) host.AvokadoContacts = api.start(host.document, host.location, host);
  ...
  const destinations = new Set(['#contacts', '#callback']);
  function createNavigation(win, doc) {
    if (!win || !doc.getElementById('contacts')) return null;
    ...
    function schedule() {
      if (!active) return;
      clearPending();
      // Let catalogue insertion, mobile details and font-driven measurements finish together.
    ...
    doc.addEventListener('avokado:catalog-ready', schedule);
  ...
  return {isSocial,start,createNavigation};
});
