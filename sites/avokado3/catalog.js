...
return `<article class="av-card${photo?' av-card--photo':''}" ${full?`id="${esc(it.id)}"`:''} data-service="${esc(it.id)}">${photo?`<img class="av-card-photo" src="${esc(photo)}" alt="" loading="lazy">`:''}<div class="av-card-body">${it.promo?'<p class="av-tag">Специальное предложение</p>':''}<h3>${full?esc(it.title):`<a href="price.html#${esc(it.id)}">${esc(it.card||it.title)}</a>`}</h3><dl class="av-facts av-facts--primary"><dt>Цена</dt><dd>${esc(it.price||'—')}${it.oldPrice?` <s>${esc(it.oldPrice)}</s>`:''}</dd><dt>Время</dt><dd>${esc(it.duration||'—')}</dd></dl>${description?`<details class="av-card-details" open><summary>Подробнее об услуге</summary><div class="av-card-details-content">${description}</div></details>`:''}${actions(data,it.id)}</div></article>`;
}
...
Promise.all([AlviPrice.load(['/api/price']),AlviPrice.load(['data/price.json'])]).then(([live,defaults])=>{const data=prepare(live,defaults);if(!data){target.innerHTML='<p class="av-loading">Прайс временно недоступен. <a href="tel:+79331901059">Уточнить у студии</a></p>';return;}target.innerHTML=render(data,full);const compact=matchMedia('(max-width:700px)');const fitCards=()=>target.querySelectorAll('.av-card-details').forEach(el=>{el.open=!compact.matches;});fitCards();compact.addEventListener('change',fitCards);if(location.hash){const el=document.getElementById(decodeURIComponent(location.hash.slice(1)));if(el&&target.contains(el))el.scrollIntoView();}});
})();
