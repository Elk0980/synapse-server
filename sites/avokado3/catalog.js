...
return `<article class="av-card${photo?' av-card--photo':''}" ${full?`id="${esc(it.id)}"`:''} data-service="${esc(it.id)}">${photo?`<img class="av-card-photo" src="${esc(photo)}" alt="" loading="lazy">`:''}<div class="av-card-body">${it.promo?'<p class="av-tag">Специальное предложение</p>':''}<h3>${full?esc(it.title):`<a href="price.html#${esc(it.id)}">${esc(it.card||it.title)}</a>`}</h3><dl class="av-facts av-facts--primary"><dt>Цена</dt><dd>${esc(it.price||'—')}${it.oldPrice?` <s>${esc(it.oldPrice)}</s>`:''}</dd>${direction==='laser'?'':`<dt>Время</dt><dd>${esc(it.duration||'—')}</dd>`}</dl>${description?`<details class="av-card-details" open><summary>Подробнее об услуге</summary><div class="av-card-details-content">${description}</div></details>`:''}${actions(data,it.id)}</div></article>`;
}
