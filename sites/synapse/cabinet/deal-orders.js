(() => {
  ...
  function showCreate(prefill={}){
    ...
    form.onsubmit=async event=>{event.preventDefault();const button=form.querySelector('[type=submit]');if(button.disabled)return;if(form.elements.companyId.disabled||!form.elements.companyId.value)return;button.disabled=true;try{const created=await query('/deals',{},ctx.csrfOptions('POST',{title:form.elements.title.value,companyId:Number(form.elements.companyId.value),requestId,...(prefill.contactId&&String(prefill.companyId)===form.elements.companyId.value?{contactId:Number(prefill.contactId)}:{})}));close();location.hash=`deals/${created.id}`;}catch(e){form.querySelector('[role=alert]').textContent=e.message;button.disabled=false;}};
  }
  ...
  function renderContacts(data){
    ...
    panel.innerHTML=data.contacts.map(c=>`<div class="order-contact"><strong>${h(c.name)}</strong><small>${h(c.role||'')} · ${c.side==='team'?'Наша команда':'Сторона клиента'}</small><div>${contactIcons(c)}${editable()?`<button type="button" data-remove-participant="${c.id}" title="Убрать из сделки" aria-label="Убрать ${h(c.name)} из сделки">×</button>`:''}</div></div>`).join('')||'<p>Участники ещё не назначены. Добавьте представителя компании, с которым ведёте эту сделку.</p>';
    ...
    panel.querySelectorAll('[data-remove-participant]').forEach(b=>b.onclick=async()=>{b.disabled=true;try{await query(`/deals/${deal.id}/participants/${b.dataset.removeParticipant}`,{},ctx.csrfOptions('DELETE'));await loadContacts();}catch(e){error(e.message);b.disabled=false;}});
  ...
  function addParticipant(){
    const id=deal.id,stamp=generation,d=modal('Участник сделки',`<form class="crm-form"><label>Контакт<select name="mode"><option value="existing">Выбрать из базы текущей компании</option><option value="new">Создать контакт</option></select></label><label data-existing>Найти по имени или телефону<input name="search" type="search"></label><label data-existing>Человек<select name="contactId"><option value="">Выберите контакт</option></select></label><label data-new hidden>ФИО<input name="personName"></label><label data-new hidden>Телефон<input name="phone" type="tel"></label><label data-new hidden>Email<input name="email" type="email"></label><label>Сторона<select name="side"><option value="client">Представитель клиента</option><option value="team">Наша команда</option></select></label><label>Роль в сделке<input name="role" value="Представитель клиента" maxlength="120" required placeholder="Собственник, директор, согласующий"></label><button type="submit">Добавить в сделку</button><p role="alert"></p></form>`);
    const f=d.querySelector('form');let sequence=0;
    const current=()=>generation===stamp&&deal?.id===id&&d.isConnected;
    const search=async()=>{const n=++sequence;try{const data=await query('/contacts',{q:f.elements.search.value,limit:50,deleted:'exclude'});if(!current()||n!==sequence)return;f.elements.contactId.innerHTML='<option value="">Выберите контакт</option>'+data.contacts.map(c=>`<option value="${c.id}">${h(c.name)}${c.phone?' · '+h(c.phone):''}</option>`).join('');}catch(e){if(current())f.querySelector('[role=alert]').textContent=e.message;}};
    f.elements.search.oninput=search;
    f.elements.mode.onchange=()=>{const fresh=f.elements.mode.value==='new';f.querySelectorAll('[data-existing]').forEach(el=>el.hidden=fresh);f.querySelectorAll('[data-new]').forEach(el=>el.hidden=!fresh);f.elements.personName.required=fresh;f.elements.contactId.required=!fresh;};
    f.elements.mode.onchange();search();
    f.onsubmit=async e=>{e.preventDefault();if(!current())return;const b=f.querySelector('[type=submit]');b.disabled=true;const body={role:f.elements.role.value,side:f.elements.side.value,...(f.elements.mode.value==='new'?{newContact:{name:f.elements.personName.value,phone:f.elements.phone.value||null,email:f.elements.email.value||null}}:{contactId:Number(f.elements.contactId.value)})};try{await query(`/deals/${id}/participants`,{},ctx.csrfOptions('POST',body));if(!current())return;d.close();d.remove();await loadContacts();}catch(err){f.querySelector('[role=alert]').textContent=err.message;b.disabled=false;}};
  }
  ...
  function nextStep(){
    ...
    else if(!deal.estimate?.length){text='Заполните смету под задачу клиента.';target='estimate';}
    ...
    return `<section class="order-stage-guide"><h2>Следующий шаг</h2><p>${h(text)}</p><button type="button" data-next-tab="${target}">Открыть нужный раздел</button>${ctx.scopeParams().companyCode==='synapse-business'?`<p>${h(growth)}</p>`:''}${deal.offer?`<p><strong>Предложение: ${h(deal.offer.label)}</strong><br>${h(deal.offer.terms)}</p>`:''}${ctx.identity.role==='owner'&&ctx.scopeParams().companyCode==='synapse-business'?'<button type="button" data-pick-offer>Выбрать модуль или пакет</button> <a href="#catalog">Настроить прайс →</a>':''}</section>`;
  }
  ...
  async function showRules(){
    try{
      ...
      const d=modal('Настроить воронку',`<form><label>Название воронки<input name="pipelineLabel" value="${h(data.label||root.querySelector('[data-order-pipeline]')?.selectedOptions[0]?.textContent||pipeline)}" maxlength="40" required></label><p>Название, этапы, цвета и критерии сохраняются только для выбранной компании. Этап со сделками удалить нельзя.</p><div data-stages>${data.stages.map(card).join('')}</div><button type="button" data-add>Добавить этап</button><button type="submit">Сохранить воронку</button><p role="alert"></p></form>`);
  ...
  cabinet.registerView('deals',{
    ...
    async render(container,context){
      ...
      if(!match){await loadList();if(location.hash.startsWith('#deals/new')){const params=new URLSearchParams(location.hash.split('?')[1]||'');showCreate({companyId:params.get('companyId'),contactId:params.get('contactId')});}return;}
