...
let ctx;
let identity, byId, escapeHTML, crmQuery, scopeParams, periodDates;
let initialized = false;
const api = {};
const init = (context) => {
  ctx = context;
  ({ identity, byId, escapeHTML, crmQuery, scopeParams, periodDates } = context);
  ...
  const loadAdPlatforms = async () => {
    ...
    try {
      ...
      if (ctx.currentView === "ad-platforms") renderAdPlatforms(dashboard);
    } catch (error) {
