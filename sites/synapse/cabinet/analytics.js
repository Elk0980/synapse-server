(() => {
  ...
  let ctx;
  let identity, selectedProjectId, currentView, byId, escapeHTML, crmQuery, csrfOptions, scopeParams, formatMoney, formatROMI, periodDates, dateValue;
  let initialized = false;
  const api = {};
  const init = (context) => {
    ctx = context;
    ({ identity, selectedProjectId, currentView, byId, escapeHTML, crmQuery, csrfOptions, scopeParams, formatMoney, formatROMI, periodDates, dateValue } = context);
    ...
    const loadAnalytics = async () => {
      if (currentView !== "analytics-through" || !identity.permissions.includes("analytics.view")) return;
