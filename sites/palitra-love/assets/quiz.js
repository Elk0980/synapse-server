(() => {
  ...
  const telegramUrl = text => `https://t.me/palitralovee?text=${encodeURIComponent(text)}`;
  ...
  document.querySelectorAll('[data-occasion-quiz]').forEach(root => {
    ...
    const directButton = '<a class="button outline quiz-direct" href="https://t.me/palitralovee" target="_blank" rel="noopener">Написать в Telegram</a>';
    ...
    const renderResult = () => {
      const productCards = config.products.map((product, index) => `<article class="quiz-product"><img src="/assets/img/${product.image}" alt="${product.name}" width="800" height="1000" loading="lazy"><div><h3>${product.name}</h3><div class="product-purchase"><p class="price">${formatPrice(product.price)}</p><a class="button product-telegram" href="https://t.me/palitralovee" target="_blank" rel="noopener">Написать в Telegram</a></div><div class="quiz-actions"><a class="button outline" data-order-product="${index}" href="#">Оформить заказ</a></div></div></article>`).join('');
