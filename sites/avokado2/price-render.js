/* Авокадо · общий рендер прайса из data/price.json.
...
${body}
  ...
  function renderLanding(data, root) {
    ...
    if (first && cards) cards.innerHTML = (first.items || []).map((it, index) => `<div class="card${index === 0 ? ' hot' : ''} rv is-in">
        <div class="tag">${esc(it.card || '')}</div>
        <h3>${esc(it.title)}</h3>
        <div class="p">${esc(it.price)}${it.oldPrice ? ` <s>${esc(it.oldPrice)}</s>` : ''}</div>
        ${it.desc ? `<p class="d">${esc(it.desc)}</p>` : ''}${it.composition ? `<p class="d" style="margin-top:10px;color:var(--brass)">${esc(it.composition)}</p>` : ''}
      </div>`).join('');
  ...
  window.AlviPrice = { esc, load, findItem, isPopular, isPromoCat, renderSections, renderNav, renderShowcase, renderLanding, blockOf: BLOCK_BY_CATEGORY };
})();
